      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {roomList.map((room) => {
          ...
          return (
            <EntityCard
              key={room.id}
              avatarInitials={<DoorOpen className="w-5 h-5" /> as any}
              avatarClass="navy-gradient text-white"
              title={room.name}
              subtitle="O'quv xonasi"
              statusBadge={<StatusBadge status={room.status} />}
              details={[
                {
                  icon: <Users2 className="w-3.5 h-3.5" />,
                  label: "Sig'imi",
                  value: `${room.capacity} o'rin`,
                },
              ]}
              progress={{
                value: pct,
                label: "Jadval bandligi",
                current: occupied,
                max: room.capacity,
              }}
              ...
              onEdit={() => { setEditRoom(room); setOpen(true); }}
              onStatusChange={(s) => updateStatus(room, s)}
              onDelete={() => deleteRoom(room)}
              deleteTitle={`"${room.name}" xonasini o'chirish`}

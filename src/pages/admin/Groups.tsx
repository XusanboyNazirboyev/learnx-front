...
const GROUP_STATUS_ACTIONS: CardStatusOption[] = [
  ...
  { value: "COMPLETED", label: "Tugatilgan", dot: "bg-violet-500" },
        ...
        {groups.map((group) => {
          ...
          return (
            <EntityCard
              ...
              title={group.name}
              subtitle={group.course?.name || "Kurs biriktirilmagan"}
              statusBadge={<StatusBadge status={group.status} />}
              details={[
                {
                  icon: <CalendarDays className="w-3.5 h-3.5" />,
                  label: "Boshlanish",
                  value: new Date(group.startDate).toLocaleDateString("uz-UZ"),
                },
                {
                  icon: <Users2 className="w-3.5 h-3.5" />,
                  label: "O'quvchilar",
                  value: `${students}/${group.maxStudent}`,
                },
              ]}
              progress={{ value: pct, label: "To'lganlik", current: students, max: group.maxStudent }}
              statusOptions={GROUP_STATUS_ACTIONS}
              currentStatus={group.status}
              onView={() => setViewGroupId(group.id)}
              onEdit={() => {
                setEditLoading(true);
              ...
              onStatusChange={(s) => updateStatus(group, s)}
              onDelete={() => deleteGroup(group)}
              deleteTitle={`"${group.name}" guruhini o'chirish`}
